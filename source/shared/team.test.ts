...
suite("notPersistedTeamSchema", () => {
	test("parsing fails when given data is undefined", () => {
		const parseResult = safeParse(notPersistedTeamSchema, undefined);

		expect(parseResult.success).toBe(false);
	});

	test("parsing fails when given data is null", () => {
		const parseResult = safeParse(notPersistedTeamSchema, null);

		expect(parseResult.success).toBe(false);
	});

	test("parsing fails when given data is not an object", () => {
		const parseResult = safeParse(notPersistedTeamSchema, "not-an-object");

		expect(parseResult.success).toBe(false);
	});

	test("parsing fails when given data is an empty object", () => {
		const parseResult = safeParse(notPersistedTeamSchema, {});
	...
	test("parsing fails when teamNumber is undefined", () => {
		...
		const parseResult = safeParse(notPersistedTeamSchema, team);
	...
	test.each([{ teamNumber: 1 }, { teamNumber: 2 }])(
		"parsing succeeds when teamNumber equals $teamNumber",
		({ teamNumber }) => {
			const team = notPersistedTeamFactory.build({ teamNumber });
			const parseResult = safeParse(notPersistedTeamSchema, team);
