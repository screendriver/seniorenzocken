...
export type ReadAudioWithoutGamePoints = Pick<ReadAudio, "gamePointAudioId" | "name">;

export type ReadAllAudiosOptions = {
...
export type AudioRepository = {
	readGamePointsAudios: (options: ReadAllAudiosOptions) => Task<readonly ReadAudio[], Error>;
	readAllFunAudios: () => Task<readonly ReadAudioWithoutGamePoints[], Error>;
};
...
export function createAudioRepository(options: AudioRepositoryOptions): AudioRepository {
	...
	return {
		readGamePointsAudios(readOptions) {
			const { team1MatchTotalGamePoints, team2MatchTotalGamePoints } = readOptions;

			return tryOrElse(
				(error: unknown) => {
					return new Error("Could not read game points audios", { cause: error });
				},
				async () => {
					return database
						.select({
		...
		readAllFunAudios() {
			return tryOrElse(
				(error: unknown) => {
					return new Error("Could not read all fun audios", { cause: error });
				},
				async () => {
					return database
						.select({
